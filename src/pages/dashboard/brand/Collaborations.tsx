    ...
    <BrandLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total Spend', value: '$1,450', color: 'text-pink-600' },
        ].map((s, i) => (
      ...
      <div className="space-y-4">
        {COLLABS.map(collab => {
          ...
          return (
            <div key={collab.id} className="bg-card rounded-2xl border border-border p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    ...
                    <Badge className={`text-[10px] shrink-0 ${status.color}`}>{status.label}</Badge>
                  </div>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                <Button variant="ghost" size="sm" className="rounded-xl h-8 text-xs gap-1" onClick={() => toast.info('Message coming soon')}>
                  <MessageSquare className="w-3 h-3" /> Message
                </Button>
                {collab.status === 'invited' && (
                  <Button size="sm" className="bg-pink-600 hover:bg-pink-700 text-white rounded-xl h-8 text-xs" onClick={() => toast.success('Collaboration confirmed!')}>
                    Confirm Deal
                  </Button>
                )}
              </div>
