    ...
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage your public artisan profile</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-2xl border border-border p-6 text-center">
            ...
            {user?.isVerified && (
              <Badge className="bg-secondary/10 text-secondary mt-2 text-xs">✓ Verified Artisan</Badge>
            )}
            <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-border">
              <div className="text-center">
                <div className="font-bold text-lg">{user?.totalSales}</div>
          ...
          <div className="bg-card rounded-2xl border border-border p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input value={form.storeName} onChange={e => setForm(f => ({ ...f, storeName: e.target.value }))} className="mt-1.5 h-10 rounded-xl" />
              </div>
              <div>
          ...
          <div className="bg-card rounded-2xl border border-border p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {['Macramé', 'Ceramics', 'Natural Dyeing', 'Weaving', 'Jewelry Making', 'Candle Making'].map(skill => (
                <Badge key={skill} variant="outline" className="rounded-full px-3 py-1 cursor-pointer hover:border-primary hover:text-primary transition-colors">
                  {skill}
                </Badge>
              ))}
