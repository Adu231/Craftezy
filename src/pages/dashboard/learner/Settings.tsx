...
export default function LearnerSettings() {
  const [notifs, setNotifs] = useState({ progress: true, newCourses: true, workshops: false, tips: false });

  return (
    <LearnerLayout>
      <div className="mb-6">
        <h1 className="font-display font-bold text-2xl sm:text-3xl">Settings</h1>
        <p className="text-muted-foreground text-sm mt-1">Manage your learning preferences</p>
      </div>
      <div className="max-w-2xl space-y-6">
        <div className="bg-card rounded-2xl border border-border p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div><h3 className="font-semibold">Notifications</h3><p className="text-xs text-muted-foreground">Learning reminders and updates</p></div>
          </div>
          <div className="space-y-4">
            {[
              { key: 'progress', label: 'Progress Reminders', desc: 'Daily study streak reminders' },
              { key: 'newCourses', label: 'New Courses', desc: 'Alerts for new courses in your interests' },
              { key: 'workshops', label: 'Workshop Updates', desc: 'Registration and session reminders' },
              { key: 'tips', label: 'Learning Tips', desc: 'Weekly craft tips and inspiration' },
            ].map(item => (
